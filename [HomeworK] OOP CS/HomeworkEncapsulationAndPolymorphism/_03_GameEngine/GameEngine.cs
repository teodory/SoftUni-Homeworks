...
namespace _03_GameEngine
{
    class GameEngine
    {
        ...
        protected virtual void CreateCharacter(string[] inputParams)
        {
            ...
            switch (inputParams[1])
            {
                case "mage":
                    ...
                    break;
                case "warrior" :
                    Warrior warior = new Warrior(id, x, y, team);
                    characterList.Add(warior);
                    break;
                case "healer" :
        ...
        {
            Character champ = GetCharacterById(inputParams[1]);
            string id = inputParams[3];

            switch (inputParams[2])
            {
                case "pill":
                    Bonus pill = new Pill(id);
                    champ.AddToInventory(pill);
                    break;
                case "injection":
                    Bonus injection = new Injection(id);
                    champ.AddToInventory(injection);
                    break;
                case "axe":
                    Item axe = new Axe(id);
                    champ.AddToInventory(axe);
                    break;
                case "shield":
                    Item shield = new Shield(id);
                    champ.AddToInventory(shield);
                    break;
            }
        ...
        {
            //Console.WriteLine(this.characterList.FirstOrDefault(x => x.Inventory.Contains(item)) + "==============");
